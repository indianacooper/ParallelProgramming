...
void parameters(int argc, char **argv) {
  ...
  char uid[L_ctermid + 2]; /*User name */
...
void initialize_inputs() {
  ...
  for (col = 0; col < N; col++) {
    for (row = 0; row < N; row++) {
      A[row][col] = (int)rand() % (20 + 1 - 1) + 1; // 32768.0;
    }
    B[col] = (int)rand() % (20 + 1 - 1) + 1; // 32768.0;
    X[col] = 0.0;
  }
...
void *pgauss(){
  int norm, row, col, i;
  float multiplier;
  // Each thread does threadpiece worth of work on the rows
  int threadpiece = N/nt;
  // sofar tracks where the next thread to start
  int sofar = threadpiece;

  for (norm = current; norm < sofar ; norm++) {
